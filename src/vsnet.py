...
class conv3DBatchNorm(nn.Module): #Defining own convolution with needed parameters because different convolutions needed
...
class conv3DBatchNormPRelu(nn.Module): #Defining own convolution with PReLU
    def __init__(self,in_channels,n_filters,k_size,stride,padding,bias=True,dilation=1,is_batchnorm=True,):
        super(conv3DBatchNormPRelu, self).__init__()
        ...
        if is_batchnorm:
            self.cbr_unit = nn.Sequential(
                conv_mod, nn.BatchNorm3d(int(n_filters)), nn.PReLU(n_filters)#Prelu
            )
        else:
...
class bottleNeckIdentity(nn.Module):
    def __init__(self, in_channels, out_channels, stride, dilation=1, is_batchnorm=True):
        super(bottleNeckIdentity, self).__init__()

        bias = True

        self.cbr1 = conv3DBatchNormPRelu(
            in_channels, out_channels, 1, stride=1, padding=0, bias=bias, is_batchnorm=is_batchnorm
        )
        self.cb3 = conv3DBatchNorm(
            out_channels, in_channels, 1, stride=1, padding=0, bias=bias, is_batchnorm=is_batchnorm
        )
    ...
    def __init__(
        ...
        if include_range in ["all", "identity"]:
            for i in range(n_blocks):
                layers.append(
                    bottleNeckIdentity(
                        out_channels, in_channels, stride, dilation, is_batchnorm=is_batchnorm
                    )
                )
...
class cascadeFeatureFusion(nn.Module):
    def __init__(
        ...
        bias = True #not is_batchnorm

        self.low_dilated_conv_bn = conv3DBatchNorm(
            low_in_channels,
            out_channels,
            (3), 
            stride=(1,1,1),
        ...
        self.high_proj_conv_bn = conv3DBatchNorm(
            high_in_channels,
            out_channels,
            (3,5,5),
            stride=1,
...
class vsnet(nn.Module):
    def __init__(
        ...
        # Encoder
        self.convbnrelu1_1 = conv3DBatchNormPRelu(
            in_channels=1,
            k_size=(3,5,5),#3
            n_filters=32,
            padding=(1,2,2),
        ...
        self.convbnrelu1_2 = conv3DBatchNormPRelu(
            in_channels=32,
            k_size=(3,5,5),#3
            n_filters=32,
            padding=(1,2,2),#1
        ...
        self.convbnrelu1_3 = conv3DBatchNormPRelu(
            in_channels=32,
            k_size=(3,5,5),#3
            n_filters=32,
            padding=(1,2,2),#1
        ...
        self.conv5_4_k1 = conv3DBatchNormPRelu(
            in_channels=32,
            k_size=(3,5,5),
            n_filters=32,
            padding=(1,2,2),
        ...
        self.convbnrelu1_sub1 = conv3DBatchNormPRelu(
            in_channels=1,
            k_size=(3,5,5),#3
            n_filters=16,
            padding=(1,2,2),
        ...
        self.convbnrelu2_sub1 = conv3DBatchNormPRelu(
            in_channels=16,
            k_size=(3,5,5),#3
            n_filters=32,
            padding=(1,2,2),
        ...
        self.convbnrelu4_sub1 = conv3DBatchNormPRelu(
            in_channels=16,
            k_size=(3,5,5),#3
            n_filters=16,
            padding=(1,2,2),
        ...
        self.convbnrelu3_sub1 = conv3DBatchNormPRelu(
            in_channels=32,
            k_size=(3,5,5),#3
            n_filters=16,
            padding=(1,2,2),
        ...
        self.convbnrelu5_sub1 = conv3DBatchNormPRelu(
            in_channels=32,
            k_size=(3,5,5),#3
            n_filters=16,
            padding=(1,2,2),
        ...
        self.cff_sub12 = cascadeFeatureFusion(
            self.n_classes, 32, 32, 32, is_batchnorm=is_batchnorm
        )
